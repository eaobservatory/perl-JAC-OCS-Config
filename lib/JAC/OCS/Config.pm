...
use JAC::OCS::Config::XMLHelper
    qw/find_children find_attr indent_xml_string get_pcdata_multi/;
...
our @CONFIGS = qw/
    obs_summary jos header rts scuba2 frontend pol
    instrument_setup tcs acsis fts2/;
...
sub acsis {
    ...
    if (@_) {
        throw JAC::OCS::Config::Error::FatalError(
            "SCUBA-2 configuration already present")
...
sub scuba2 {
    ...
    if (@_) {
        throw JAC::OCS::Config::Error::FatalError(
            "ACSIS configuration already present")
            if defined $self->acsis();
        throw JAC::OCS::Config::Error::FatalError(
            "Heterodyne frontend configuration already present")
            if defined $self->frontend();

        $self->{SCUBA2_CONFIG} = check_class_fatal("JAC::OCS::Config::SCUBA2", shift);
    }
...
sub frontend {
    ...
    if (@_) {
        ...
        $self->{FRONTEND_CONFIG} = check_class_fatal("JAC::OCS::Config::Frontend", $fe);
...
sub write_file {
    ...
    for my $dir (File::Spec->curdir, @dirs) {
        ...
        open(my $fh, "> $fullname")
            ...
            "Error opening config output file $fullname: $!");

        # Now we use the inverse map and full config list to select
        # specific configurations for this directory
        my %strargs;
        if ($dir ne File::Spec->curdir
            && exists $invtmap->{$dir}
            && !exists $full_configs{$dir}
        ) {
        ...
        close($fh)
            or throw JAC::OCS::Config::Error::IOError(
            "Error closing config output file $fullname: $!");
...
sub duration_scuba2 {
    ...
    my $jos = $self->jos;
    throw JAC::OCS::Config::Error::FatalError(
        "Unable to determine duration since there is no JOS configuration available")
        unless defined $jos;

    # Get observation summary
    my $obssum = $self->obs_summary;
    throw JAC::OCS::Config::Error::FatalError(
        "Unable to determine duration since there is no observation summary available")
        unless defined $obssum;
    ...
    if ($obs_type ne "flatfield"
        ...
        $tcs = $self->tcs;
        throw JAC::OCS::Config::Error::FatalError(
            "Unable to determine duration since there is no telescope configuration")
            unless defined $tcs;
        ...
        $oa = $tcs->getObsArea;
        throw JAC::OCS::Config::Error::FatalError(
            "Unable to determine duration since there is no observing area configuration")
            unless defined $oa;
    }
    ...
    if ($obs_type eq 'skydip') {
        ...
        if ($map_mode eq 'scan') {
            ...
            JAC::OCS::Config::Error::FatalError->throw(
                "Unknown map mode for SCUBA-2 skydip: '$map_mode'");
        }
        ...
        if ($jos->jos_min() > 1) {
            # special case - this is simply the time we are going to spend
            # on each "map" between darks
            $steps_per_map = $jos->jos_min;

        }
        elsif ($pattern =~ /raster|bous/i) {
            # make an estimate of the number of samples in the map area.
            # Add one array diameter to longest dimension
            # and also add an extra 60 arcsec turn around area.
            my $inst = $self->instrument_setup();
            throw JAC::OCS::Config::Error::FatalError(
                "Unable to determine duration since there is no instrument configuration")
                unless defined $inst;
            ...
            JAC::OCS::Config::Error::FatalError->throw(
                "Scan pattern '$pattern' not recognized");
        }
        ...
        JAC::OCS::Config::Error::FatalError->throw(
            "Unrecognized observing mode for duration calculation: '$map_mode/$obs_type'");
    }
    ...
    print "NDARKS=$ndarks  NSEQ= $nseq  TIME/SEQ="
        . ($eff_step_time * $time_per_seq) . "\n"
        if $DEBUG;
    ...
    my $duration = $startup_overhead
        + (($ndarks + $nseq) * $seq_start_overhead)
        + ($time_per_seq * $nseq)
        + ($ndarks * $darklen);
...
sub duration_acsis {
    ...
    if ($obs_type =~ /skydip/) {
        ...
        # consistency check
        my $mode = $obsarea->mode;
        throw JAC::OCS::Config::Error::FatalError(
            "Inconsistency in configuration. Scan map requested but obsArea does not specify a map area (mode='$mode' not 'area')")
            unless $mode =~ /area/i;

        # Need to work out the number of samples in the map

        # Area of map
        my %mapdims = $obsarea->maparea();
        my $maparea = $mapdims{HEIGHT} * $mapdims{WIDTH};

        # Area of each "sample point"
        my %scan = $obsarea->scan;
        my $dx = $scan{VELOCITY} * $step_time;
        my $dy = $scan{DY};
        my $samparea = $dx * $dy;

        # Number of sample points
        $num_steps = $maparea / $samparea;

        print "Number of steps for map = $num_steps ($dx x $dy in $mapdims{HEIGHT} x $mapdims{WIDTH})\n"
            if $DEBUG;
        ...
        if ($mapdims{HEIGHT} == $mapdims{WIDTH}) {
            ...
            my $map_pa = $obsarea->posang->degrees;
        ...
        my $num_scans = ceil($ysize / $scan{DY});

        # number of steps in a row
        my $row_steps = ceil($rowlen / $dx);

        # number of rows per refs (must be smaller than steps_btwn_refs
        # but at least 1.
        my $nrows_per_ref = int($jos->steps_btwn_refs / $row_steps);
        ...
        $num_refs = ceil($num_scans / $nrows_per_ref);

        # Convert to number of reference steps
        $ref_steps = ceil(sqrt($nrows_per_ref * $row_steps));
        ...
        $num_tel_ref_moves = (2 * $num_refs) - 1;

        # at least 2
        $num_tel_ref_moves = 2 if $num_tel_ref_moves < 2;
        ...
        my @offsets = $obsarea->offsets;
        my $num_offsets = scalar(@offsets);

        throw JAC::OCS::Config::Error::FatalError(
            "Unable to determine duration since there is no secondary mirror information")
            unless defined $secondary;

        # and the number of points in the jiggle pattern
        my $jig = $secondary->jiggle;
        my $num_jig_pts = $jig->guess_npts;
        ...
        if ($sw_mode eq 'chop') {
            ...
            print "No nodding\n" if $DEBUG;
            $num_seq = $jos->num_cycles;

            # 2 switches for frequency switch
            $num_steps *= 2 * $jos->num_cycles;
        }
        else {
            throw JAC::OCS::Config::Error::FatalError(
                "Unable to determine duration since there is an unexpected switch mode in jiggle: $sw_mode");
        }
    }
        ...
        $num_steps = $jos->jos_mult * 2 * $jos->num_cycles;

        # Number of sequence starts is just the number of offsets
        $num_seq = $jos->num_cycles * $num_offsets;
    }
        ...
        if ($jos->shareoff) {
            if ($map_mode =~ /jiggle/) {
                ...
                $ref_steps = ($jos->jos_min / $num_jig_pts) * sqrt($num_jig_pts);
            }
            else {
                # timing depends on the number of offsets we did per ON
                $ref_steps = $jos->jos_min * sqrt($num_chunks_per_ref);
            }
        }
        else {
    ...
    my $duration = ($steps_per_cyc * $step_time)   # on+off time
        + ($num_tel_ref_moves * $tel_ref_overhead) # number of refs
        ...
        if (defined $tcs) {
            ...
            if (!defined $c) {
                if ($obs =~ /(setup|skydip|noise)/i) {
                    ...
                    if (keys %tags) {
                        throw JAC::OCS::Config::Error::NeedNextTarget(
                            "No science target defined in configuration");
                    }
                }
                else {
                    throw JAC::OCS::Config::Error::MissingTarget(
                        "No science target defined in configuration");
                }
            }
        }
            ...
            throw JAC::OCS::Config::Error::FatalError(
                "No TCS definition available in this configuration");
        }
    }
}
...
sub fixup {
    ...
    my $tcs = $self->tcs;
    push @messages, $tcs->fixup($self->duration(), $self->jos())
        if defined $tcs;
...
sub stringify {
    ...
    if (defined $args{CONFIGS}) {
        ...
        for my $c (@{$args{CONFIGS}}) {
            throw JAC::OCS::Config::Error::FatalError(
                "Supplied configuration method '$c' is not supported")
                unless $self->can($c);
            my $object = $self->$c;
...
sub _process_dom {
    ...
    $self->header(new JAC::OCS::Config::Header(DOM => $cfg))
        if $cfg;
    ...
    $self->tcs(new JAC::OCS::Config::TCS(DOM => $cfg, %hlp))
        if $cfg;

    $cfg = find_children($el, "ACSIS_CONFIG", min => 0, max => 1);
    $self->acsis(new JAC::OCS::Config::ACSIS(DOM => $cfg))
        if $cfg;

    $cfg = find_children($el, "SCUBA2_CONFIG", min => 0, max => 1);
    $self->scuba2(new JAC::OCS::Config::SCUBA2(DOM => $cfg))
        if $cfg;
    ...
    $self->frontend(new JAC::OCS::Config::Frontend(DOM => $cfg))
        if $cfg;

    $cfg = find_children($el, "RTS_CONFIG", min => 0, max => 1);
    $self->rts(new JAC::OCS::Config::RTS(DOM => $cfg))
        if $cfg;

    $cfg = find_children($el, "POL_CONFIG", min => 0, max => 1);
    $self->pol(new JAC::OCS::Config::POL(DOM => $cfg))
        if $cfg;

    $cfg = find_children($el, "FTS2_CONFIG", min => 0, max => 1);
    $self->fts2(new JAC::OCS::Config::FTS2(DOM => $cfg))
        if $cfg;
