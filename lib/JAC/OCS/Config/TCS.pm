...
SpTelescopeObsComp element (TOML) or TCS_CONFIG element (JAC
configuration file).
...
sub new {
  ...
  if (exists $args{DOM} && defined $args{DOM}) {
    ...
    croak "Arguments supplied to constructor but not recognized";
  } else {
    croak "Must supply arguments to constructor\n";
  }
  return $cfg;
}
...
object. Does not necessarily refer to the beginning of the TCS XML
segment of the parse tree (will normally refer to the beginning of
whatever XML was passed into the constructor).
...
Node in the parse tree corresponding to the start of the TCS configuration
XML.
...
Import a string containing the TCS XML into the object.
...
Throws an exception if TCS information can not be found in the
tree.
...
sub _import_dom {
  ...
  my @nodes = $tree->findnodes("//TCS_CONFIG | //SpTelescopeObsComp");

  croak "DOM contains multiple TCS configurations!"
    if scalar(@nodes) > 1;

  croak "DOM contains no TCS configurations!"
    if !scalar(@nodes);

  # found some configuration XML. Store it
  $self->_tree( $tree );
  $self->_tcsnode( $nodes[0] );
...
=item B<_process_tcs_dom>

Using the C<_tcsnode> node referring to the top of the TCS XML, process the DOM tree
and extract all the coordinate information.
