...
use JAC::OCS::Config::XMLHelper
    qw/find_children find_attr indent_xml_string/;
...
sub item {
    ...
    if ($arg =~ /^[0-9]+$/) {
        ...
        if (ref($arg) eq 'Regexp') {
            ...
            @match =
                grep {defined $_->keyword && $_->keyword eq $arg} @items;
        }
...
sub set_ocscfg_filename {
    ...
    my @items = $self->item(
        sub {
            (defined $_[0]->keyword && $_[0]->keyword eq $magic)
                || (defined $_[0]->method && $_[0]->method eq "get$magic");
        }
    );
    ...
    for my $i ($self->items) {
        ...
        if (exists $modifiers{$k}) {
            ...
            if (!defined $modifiers{$k}) {
                ...
                $i->comment($updated_info{COMMENT})
                    if exists $updated_info{COMMENT};
            }
        }
    }
...
sub read_header_exclusion_file {
    ...
    open my $fh, '<', $xfile
        ...
        "Error opening exclusion file '$xfile': $!");
...
sub remove_excluded_headers {
    ...
    for my $ex (@{$toexclude}) {
        ...
        if (defined $item) {
            __PACKAGE__->_print_fh((sprintf "\tClearing header %s\n", $ex), $outh)
                if $verbose;

            $item->undefine;
        }
        else {
            __PACKAGE__->_print_fh((sprintf "\tAsked to exclude header card '%s' but it is not part of the header\n", $ex), $outh)
                if $verbose;
        }
    }
...
sub verify_header_types {
    ...
    for my $ocs_h ($self->items) {
        ...
        my $v = $ocs_h->value;
        next unless defined $ocs_h->source || (defined $v && length $v);
    ...
    for my $a (@items) {
        ...
        if ($name =~ /_INCLUDE/) {
            ...
            throw JAC::OCS::Config::Error::FatalError(
                "Odd internal error in HEADER_CONFIG parse");
        }
        ...
        for my $i (@subitems) {
            ...
            push @obj, JAC::OCS::Config::Header::Item->new(%attr, %mon);
        }
    }
...
sub _read_source_defs {
    ...
    my @lines = <$fh>;
    close($fh)
        or JAC::OCS::Config::Error::IOError->throw("Error closing file '$file': $!");
    ...
    for my $l (@$lref) {
        ...
        if ($command eq 'INCLUDE') {
            ...
            JAC::OCS::Config::Error::XMLBadStructure->throw(
                "Unrecognized format for line '$l'")
                unless @parts > 1;
            ...
            # Apply task mapping
            if (($item{SOURCE} eq 'DERIVED' || $item{SOURCE} eq 'DRAMA')
                    && exists $taskmap{$item{TASK}}) {
