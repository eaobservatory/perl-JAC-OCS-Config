...
sub stringify {
    ...
    throw JAC::OCS::Config::Error::FatalError(
        'Must supply rest frequency in order to create XML')
        unless defined $rfreq;

    $xml .= "<REST_FREQUENCY>" . $rfreq . "</REST_FREQUENCY>\n";

    $xml .= "<FREQ_OFF_SCALE>"
        . (defined $self->freq_off_scale ? $self->freq_off_scale : 0)
        . "</FREQ_OFF_SCALE>\n";

    throw JAC::OCS::Config::Error::FatalError(
        'Must supply sideband in order to create XML')
        unless defined $self->sideband;
...
sub _process_dom {
    ...
    for my $k (@names) {
        throw JAC::OCS::Config::Error::XMLEmpty(
            "FRONTEND_CONFIG is missing element '$k'")
            unless exists $mandatory{$k};
        my $method = lc($k);
