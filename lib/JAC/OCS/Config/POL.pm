...
use JAC::OCS::Config::XMLHelper qw/
    find_children find_attr indent_xml_string get_pcdata/;
...
sub stringify {
    ...
    if ($is_spin) {
        ...
        throw JAC::OCS::Config::Error::FatalError(
            "No spin speed defined for pol configuration")
        ...
        if (defined $steps_per_rot && $steps_per_rot > 0) {
            ...
            my @pa = $self->discrete_angles;
            throw JAC::OCS::Config::Error::FatalError(
                "Attempting to write a step and integrate pol configuration without specifying any angles")
                unless @pa;
...
sub _process_dom {
    ...
    if ($args{NAME} eq 'CONSTANT') {
        ...
        throw JAC::OCS::Config::Error::FatalError(
            "Looking for continuous spin POL_CONFIG but did not find rotation speed")
            unless defined $speed;

    }
    elsif ($args{NAME} eq 'STOP_START') {
        # Can be either an array of PA or a STEPS_PER_ROT element
        my $s_per_rot = find_children($pol_spin, "STEPS_PER_ROT", min => 0, max => 1);
        ...
        if (defined $s_per_rot) {
            ...
            @pa = map {Astro::Coords::Angle->new($delta * $_, units => 'deg')}
                0 .. ($nsteps - 1);
        ...
        throw JAC::OCS::Config::Error::FatalError(
            "Parsing Stop/Start POL_CONFIG but found no angle definitions")
            unless @pa;
        ...
        throw JAC::OCS::Config::Error::FatalError(
            "Unknown name '$args{NAME}' in POL_SPIN attributes");
    }
