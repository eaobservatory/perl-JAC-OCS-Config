...
use JAC::OCS::Config::XMLHelper qw/
    find_children find_attr indent_xml_string get_this_pcdata/;
...
sub stringify {
    ...
    for my $id (sort {
        ...
        $xml .= "molecule=\"" . $lines{$id}->molecule . "\" "
            if defined $lines{$id}->molecule;
        $xml .= "transition=\"" . $lines{$id}->transition . "\" "
            if defined $lines{$id}->transition;
        $xml .= ">\n";
...
Convert a molecular species and transition into a key suitable for storing
...
sub _process_dom {
    ...
    for my $rf (@rfreq) {
        ...
        $lines{$attr{id}} = JAC::OCS::Config::ACSIS::Line->new(
            RestFreq => $freq,
