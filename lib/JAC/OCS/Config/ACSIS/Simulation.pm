...
use JAC::OCS::Config::XMLHelper qw/
    find_children find_attr get_pcdata_multi indent_xml_string/;
    ...
    if (@_) {
        if (ref($_[0]) eq 'ARRAY') {
            ...
            throw JAC::OCS::Config::Error::BadArgs(
                "Argument supplied to clouds() neither hash nor array ref");
        }
    }
    return @{$self->{CLOUDS}};
}
...
sub stringify {
    ...
    for my $c (@clouds) {
        $xml .= "<cloud "
            . (exists $c->{spw_id} ? "spw_id=\"" . $c->{spw_id} . "\" " : "")
            . ">\n";
