...
use JAC::OCS::Config::XMLHelper qw/
    get_pcdata get_pcdata_multi find_attr indent_xml_string/;
use JAC::OCS::Config::TCS::Generic qw/
    coords_to_xml offset_to_xml find_offsets/;
...
sub from_coord {
    ...
    if ($object->isa("Astro::Coords")) {
        ...
        throw JAC::OCS::Config::Error::BadArgs(
            "Supplied coordinate to from_coord() is neither and Astro::Coords nor JAC::OCS::Config::TCS::BASE");
    }
...
sub coords {
    ...
    if (@_) {
        ...
        throw JAC::OCS::Config::Error::FatalError(
            "Coordinate object is not of correct class")
...
sub offset {
    ...
    if (@_) {
        ...
        my $arg = shift;
        throw JAC::OCS::Config::Error::FatalError(
            "Offset object is not of correct class")
        ...
        if ($ts eq 'TRACKING') {
            ...
            if (exists $syslut{$native}) {
                ...
                $xml .= "<!-- Tracking system was TRACKING and native coord system was "
                    . (defined $native ? $native : "undefind")
                    . " but the translator does not understand this so choosing J2000 -->\n";
                $ts = 'J2000';
            }
        }
...
sub _find_base_posn {
    ...
    if ($name eq 'BASE') {
        ...
        throw JAC::OCS::Config::Error::XMLBadStructure(
            "Can find neither 'BASE' nor 'base' element!");
    }
    throw JAC::OCS::Config::Error::XMLBadStructure(
        "Unable to find tag associated with this base position")
        unless defined $tag;
    ...
    throw JAC::OCS::Config::Error::XMLBadStructure(
        "Unable to find target inside BASE element\n")
        unless $target;
...
sub _extract_coord_info {
    ...
    throw JAC::OCS::Config::Error::FatalError(
        "Unable to determine the coordinate system. Have you included a reference to the TCS DTD?")
        if !defined $type;

    # hmsdeg and degdeg are old variants of spherSystem
    if ($sysname eq "hmsdegSystem"
            or $sysname eq "degdegSystem"
        ...
        if ($type =~ /^[BJ]\d\d\d\d/) {
            ...
            throw JAC::OCS::Config::Error::FatalError(
                "HA/Dec requires a telescope but no telescope is defined!")
                unless defined $tel;
            ...
            throw JAC::OCS::Config::Error::FatalError(
                "Do not know how to parse '$sysname' coordinates of type '$type'");
        }
        ...
        $c = new Astro::Coords(%coords, name => $name);

        throw JAC::OCS::Config::Error::FatalError(
            "Error reading coordinates from XML for target $name / system $sysname. Tried "
                . Dumper(\%coords))
            unless defined $c;
        ...
        throw JAC::OCS::Config::Error::FatalError(
            "Error reading coordinates from XML for target $name. Tried elements"
                . Dumper(\%elements))
            unless defined $c;
        ...
        throw JAC::OCS::Config::Error::FatalError(
            "No planet name supplied for namedSystem")
            unless $name;
        ...
        if (grep {$_ != 0.0} (
                $epoch_year, $epoch_day, $inclination,
                $raanode, $perigee, $e,
                $m_anomaly, $m_motion, $bstar)) {
            ...
            $c = new JAC::OCS::Config::Coords::AutoTLE(name => $name);
        }

        throw JAC::OCS::Config::Error::FatalError(
            "Error reading coordinates from XML for target $name as TLE")
            unless defined $c;
    }
    else {
        throw JAC::OCS::Config::Error::FatalError(
            "Target system ($sysname) not recognized\n");
    }
