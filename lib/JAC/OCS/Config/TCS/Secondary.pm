...
use JAC::OCS::Config::XMLHelper qw/
    find_children find_attr get_pcdata get_this_pcdata indent_xml_string/;
use JAC::OCS::Config::TCS::Generic qw/
    find_pa find_offsets pa_to_xml/;
...
a C<E<lt>SECONDARYE<gt>> element. It will not attempt to verify that it has
...
sub smu_mode {
    ...
    if (%c && $j) {
        ...
        if (exists $timing{CHOPS_PER_JIG}
                && defined $timing{CHOPS_PER_JIG}
        ...
        if ($mode eq 'chop' || $mode eq 'chop_jiggle') {
            ...
            if (!exists $timing{N_JIGS_ON} && !exists $timing{N_CYC_OFF}) {
                JAC::OCS::Config::Error::FatalError->throw(
                    "Meant to be jiggle/chop but not enough information");
            }
    ...
    if ($mode eq 'dream') {
        ...
        if ($mode eq 'jiggle'
            ...
            throw JAC::OCS::Config::Error::FatalError(
                "We have a jiggle configuration ($mode) but no jiggle information!\n")
                unless defined $j;
    ...
    if (defined $jchop) {
        ...
        if ($cpj) {
            ...
            throw JAC::OCS::Config::Error::XMLBadStructure(
                "Timing indicates CHOPS_PER_JIG but no content available")
                unless defined $timing{CHOPS_PER_JIG};
        }
        elsif ($jpc) {
            # Attributes
            ...
            throw JAC::OCS::Config::Error::XMLBadStructure(
                "JIGGLE_CHOP must have either CHOPS_PER_JIG or JIGS_PER_CHOP defined");
        }
        $self->timing(%timing);
    }
}
