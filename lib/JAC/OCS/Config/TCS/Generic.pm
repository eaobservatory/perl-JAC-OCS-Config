...
our @EXPORT_OK = qw/find_offsets find_pa pa_to_xml offset_to_xml coords_to_xml/;
...
sub find_offsets {
    ...
    for my $o (@matches) {
        ...
        $opt{projection} = $opt{TYPE}
            if (exists $opt{TYPE} && defined $opt{TYPE});
...
sub coords_to_xml {
    ...
    if ($type eq "PLANET") {
        ...
        if ($type eq 'RADEC') {
            ...
            throw JAC::OCS::Config::Error::FatalError(
                "Completely impossible - type neither RADEC nor FIXED but $type\n");
        }
        $xml .= "  </spherSystem>\n";
    }
        ...
        if (exists $el{DM}) {
            ...
            throw JAC::OCS::Config::Error::FatalError(
                "Unable to determine element type from elements!");
        }
        ...
        throw JAC::OCS::Config::Error::FatalError(
            "Do not yet know how to xml-ify coords of type $type");
    }
